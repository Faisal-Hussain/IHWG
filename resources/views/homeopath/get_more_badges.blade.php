        ...
        <div class="card-content">
                ...
                <div class="permissions mt-2">
                    ...
                    @foreach(badges() as$key=> $item)
                        ...
                        <input type="radio" name="badge_id" value="{{ $item->id }}" id="{{ $item->id }}" required="" class="badge-redio" data-src="{{ asset($item->path) }}" data-name="{{$item->title}}">
                        <label for="{{ $item->id }}">{{ $item->title }}</label>
                        
                            @if($key==1)
                            <img src="{{ asset('uploads/badges/default_badge.png') }}" class="w-25 ml-auto pull-right badge-img">
